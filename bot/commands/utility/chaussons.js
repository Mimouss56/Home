...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('chaussons')
    .setDescription('Rôle secret des Quarks de O\'Clock')
  ...
  async execute(interaction) {
    ...
    if (response === 'particule') {
      ...
      const hasquarksRole = interaction.guild.roles.cache.find((role) => role.id === quarksRole);
      if (member.roles.cache.has(hasquarksRole.id)) { // Vérifie si l'utilisateur a le rôle "Quarks"
        ...
        await interaction.member.roles.add([quarksRole]);
