...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (response === 'particule') {
      ...
      const hasquarksRole = interaction.guild.roles.cache.find((roleDetails) => roleDetails.id === roleDetails.quarks);
      if (member.roles.cache.has(hasquarksRole.id)) {
        // Vérifie si l'utilisateur a le rôle "Quarks"
