...
module.exports = {
  ...
  /**
   *
   * @param {number} discordId ID discord du sanctionné
   ...
   * @returns {returnJson}
   */
  async createSanction(discordId, typeId, reason, authorId, guildId) {
    const token = await optionService.getOne('BBC');
    discord.SetAccessInfo('Bot', token.BBC); let infoMethod = 'GET';
